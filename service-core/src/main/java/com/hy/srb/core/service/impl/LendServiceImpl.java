...
import com.hy.srb.core.pojo.entity.*;
...
import com.hy.srb.core.service.*;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.hy.srb.core.utils.*;
...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void createLend(BorrowInfoApprovalVO borrowInfoApprovalVO, BorrowInfo borrowInfo) {
        ...
        //******* 平台收益 = 标的金额 * 月年化 * 期数
        BigDecimal expectAmount = lend.getAmount().multiply(monthRate).multiply(new BigDecimal(lend.getPeriod().toString()));
...
//          （6）生成借款人还款计划和出借人回款计划
        ...
        BigDecimal realAmount = lend.getInvestAmount().multiply(monthRate).multiply(new BigDecimal(lend.getPeriod().toString()));
        ...
        //给借款账号转入金额
        BigDecimal total = new BigDecimal(result.getString("voteAmt")); //放款时的金额需要减去平台的手续费
    ...
    private void repaymentPlan(Lend lend) {
        ...
        //获取放款时创建的LendReturn还款计划列表中的 创建时间create_time
        /*QueryWrapper<LendReturn> lendReturnQueryWrapper = new QueryWrapper<>();
        lendReturnQueryWrapper.eq("lend_id", lend.getId());
        List<LendReturn> lendReturns = lendReturnService.list(lendReturnQueryWrapper);*/
    ...
    /**
     * 回款计划
     *获取某个投资人的投资信息LendItem，生成每期（每月）的回款计划，封装到 lendItemReturnList
     * @param lendItemId
    ...
    public List<LendItemReturn> returnInvest(Long lendItemId, Map<Integer, Long> lendReturnMap, Lend lend) {
        ...
        Long lendReturnId=0l;
        for (Map.Entry<Integer, BigDecimal> entry : mapInterest.entrySet()) {
            Integer currentPeriod = entry.getKey();
            //根据还款期数获取还款计划的id
            lendReturnId = lendReturnMap.get(currentPeriod);
//            lendReturnMap.get()
            ...
            if (lendItemReturnList.size() > 0 && currentPeriod.intValue() == lend.getPeriod().intValue()) {
                ...
                BigDecimal sumInterest = lendItemReturnList.stream()
                        .map(LendItemReturn::getInterest)
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                //最后一期应还利息 = 用当前投资人的总投资收益（总利息） - 除了最后一期前面期数计算出来的所有的应还利息
        ...
        lendItemReturnQueryWrapper.eq("invest_user_id", lendItem.getInvestUserId())
                .eq("lend_id", lendItem.getLendId())
                .eq("lend_item_id", lendItem.getId());
        List<LendItemReturn> lendItemReturnList1 = lendItemReturnService.list(lendItemReturnQueryWrapper);
        return lendItemReturnList1;
    }
